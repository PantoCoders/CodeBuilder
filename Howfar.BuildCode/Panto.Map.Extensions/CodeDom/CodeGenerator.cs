    ...
    internal class CodeGenerator
    {
        ...
        public CodeGenerator(Type entityType)
        {
            ...
            foreach (PropertyInfo property in _entityType.GetProperties(BindingFlags.Instance | BindingFlags.Public))
            {
                ...
                ExtendColumnAttribute attrExtColumn = property.GetMyAttribute<ExtendColumnAttribute>();

                // 扩展的字段跳过 by chenb02
                //if (attrExtColumn != null)
                //    continue;
                ...
                if (attrColumn != null)
                {
                    ...
                    info = new DbMapInfo(fieldName, property.Name, attrColumn, attrExtColumn, property);
                    _allFields.Add(new DbMapInfo(fieldName, property.Name, attrColumn, attrExtColumn, property));
                    ...
                    _allFields.Add(new DbMapInfo(fieldName, property.Name, null, attrExtColumn, property));
            }
...
namespace _Tool.AutoGenerateCode {";
        ...
        public string GetCode(string className)
        {
            ...
            _sb.AppendFormat("private static List<FieldValid> _fieldValid = GetValids(typeof({1}));{0}", Environment.NewLine, _entityType.FullName);
        ...
        private void Switch()
        {
            ...
            _sb.AppendFormat("case 14: return Valid(({0})parameters[0], (List<String>)parameters[1]);{1}", _entityType.FullName, Environment.NewLine);
        ...
        private void Insert()
        {
            ...
            _sb.AppendLine("string[] zeroProperties = t1.ChangedProperties();");
            ...
            _sb.AppendFormat("sqlBuilder.Append(\"insert into {0} (\");\r\n", _tableInfo.DbName);
            _sb.AppendFormat("t1.ClearUpdateField();");

            foreach (DbMapInfo info in _allFields)
            {
                if (info.AttrColumn == null || (info.AttrColumn != null && (info.AttrColumn.Identity || info.AttrColumn.TimeStamp || info.AttrColumn.SeqGuid)))
                    continue;

                // 扩展的字段跳过 by chenb02
                if (info.AttrExtColumn != null)
                    continue;

                _sb.AppendFormat("t1.AddUpdateField(\"" + info.DbName + "\");");
        ...
        private string OutPropertyGetValue(DbMapInfo info, string varName)
        {
            ...
            {
                return string.Format("(int){0}.{1}", varName, info.NetName);
            }
        ...
        private void UpdateSetFields()
        {
            ...
            _sb.AppendLine("bool changed = false;");
            _sb.AppendLine("t1.ClearUpdateField();");
            _sb.AppendLine("string[] zeroProperties = t1.ChangedProperties();");
            ...
            foreach (DbMapInfo info in _allFields)
            {
                if (info.AttrColumn == null || (info.AttrColumn != null && (info.AttrColumn.Identity || info.AttrColumn.TimeStamp || info.AttrColumn.PrimaryKey)))
                    continue;
                ...
                if (info.PropertyInfo.PropertyType.IsClass && info.PropertyInfo.PropertyType != typeof(string))
                    ...
                    _sb.AppendFormat("if( true ){{\r\n");		// 枚举类型
                else if (info.PropertyInfo.PropertyType.IsValueType
                    /*&& info.PropertyInfo.PropertyType.IsNullableType() == false*/ )
                    _sb.AppendFormat("if( t1.{0} != t2.{0} || Array.IndexOf(zeroProperties, \"{0}\") >= 0){{\r\n", info.NetName);
                else
                    _sb.AppendFormat("if( t1.{0} != t2.{0} ){{\r\n", info.NetName);		// 引用类型或者可空类型不可能是零值
...
_timeStampField.DbName, "original_", "t2", _timeStampField.NetName);
        ...
        private void WhereByOriginalValue()
        {
            ...
            _sb.AppendLine("string[] zeroProperties = t2.ChangedProperties();");
            _sb.AppendLine("query=query+ \" where \";");
            ...
            foreach (DbMapInfo info in _allFields)
            {
                if (info.AttrColumn != null && (info.AttrColumn.PrimaryKey || info.AttrColumn.TimeStamp))
                    continue;
                ...
                if (info.AttrColumn != null && info.AttrColumn.IsNullable)
...
public static byte[] LongToByte(long value){
	return ReverseArray(BitConverter.GetBytes(value));
        ...
        private void GetValids()
        {
            _sb.AppendFormat("private static List<FieldValid> GetValids(Type type){0}{{{0}List<FieldValid> Valids = new List<FieldValid>();{0}ValidAttribute[] attr;{0}List<IValid> v;{0}", Environment.NewLine, _entityType.FullName);
            _sb.AppendFormat("IValid t;{0}foreach (PropertyInfo pi in type.GetType().GetProperties()){0}{{{0}attr = ((ValidAttribute[])pi.GetCustomAttributes(typeof(ValidAttribute), true));{0}", Environment.NewLine, _entityType.FullName);
            _sb.AppendFormat("v = new List<IValid>();{0}for (int i = 0; i < attr.Length; i++){0}{{", Environment.NewLine, _entityType.FullName);
            _sb.AppendFormat("t = attr[i] as IValid;{0}if (t != null){0}{{{0}v.Add(t);{0}}}{0}}}{0}", Environment.NewLine, _entityType.FullName);
            _sb.AppendFormat("if (v.Count > 0){0}{{{0}Valids.Add(new FieldValid(pi, v));{0}}}{0}}}{0}return Valids;{0}}}{0}", Environment.NewLine, _entityType.FullName);
        }

        private void Valid()
        {
            _sb.AppendFormat("public static Boolean Valid({1} entity, List<String> fields){0}{{{0}List<FieldValid> fieldValids = entity.GetFieldValid();List<String> ChangedProperties = entity.GetUpdateField();", Environment.NewLine, _entityType.FullName);
            _sb.AppendFormat("foreach (FieldValid ve in fieldValids){0}{{{0}if(fields.Contains(ve.Property.Name)){{continue;}} if(ChangedProperties.Contains(ve.Property.Name))", Environment.NewLine, _entityType.FullName);
            _sb.AppendFormat("{0}{{{0}foreach (IValid v in ve.List){0}{{{0}if (!v.Valid(ve.Property.GetValue(entity, null))){0}{{{0}throw new Exception(v.Message);{0}}}{0}}}{0}}}{0}}}{0}return true;{0}}}{0}", Environment.NewLine);
        }

        private static Dictionary<string, string> s_dictXmlSetCode = new Dictionary<string, string>(13){
			{"System.Int64", "obj.SetPropertyDefaultValue(\"{0}\");if (string.IsNullOrEmpty(value)){{return;}} long tmp;if(long.TryParse(value, out tmp)){{obj.{0}=tmp;}}\r\n"},
			...
			{"System.Boolean","obj.SetPropertyDefaultValue(\"{0}\");if (string.IsNullOrEmpty(value)){{return;}} bool tmp;if (bool.TryParse(value, out tmp)){{obj.{0}=tmp;}}\r\n"},
			{"System.String","if (value == null){{return;}}\r\n obj.{0} = value;\r\n"},
			{"System.DateTime","obj.SetPropertyDefaultValue(\"{0}\");if (string.IsNullOrEmpty(value)){{return;}} DateTime tmp;if(DateTime.TryParse(value, out tmp)){{obj.{0}=tmp;}};\r\n"},
			{"System.Decimal","obj.SetPropertyDefaultValue(\"{0}\");if (string.IsNullOrEmpty(value)){{return;}} decimal tmp;if(decimal.TryParse(value, out tmp)){{obj.{0}=tmp;}}\r\n"},
			{"System.Double","obj.SetPropertyDefaultValue(\"{0}\");if (string.IsNullOrEmpty(value)){{return;}} double tmp;if(double.TryParse(value, out tmp)){{obj.{0}=tmp;}}\r\n"},
			{"System.Int32","obj.SetPropertyDefaultValue(\"{0}\");if (string.IsNullOrEmpty(value)){{return;}} int tmp;if(int.TryParse(value, out tmp)){{obj.{0}=tmp;}}\r\n"},
			{"System.Single","obj.SetPropertyDefaultValue(\"{0}\");if (string.IsNullOrEmpty(value)){{return;}} float tmp;if(float.TryParse(value, out tmp)){{obj.{0}=tmp;}}\r\n"},
			{"System.Int16","obj.SetPropertyDefaultValue(\"{0}\");if (string.IsNullOrEmpty(value)){{return;}} short tmp;if(short.TryParse(value, out tmp)){{obj.{0}=tmp;}}\r\n"},
			{"System.Byte","obj.SetPropertyDefaultValue(\"{0}\");if (string.IsNullOrEmpty(value)){{return;}} byte tmp;if(byte.TryParse(value, out tmp)){{obj.{0}=tmp;}}\r\n"},
			{"System.Guid","obj.SetPropertyDefaultValue(\"{0}\");if (string.IsNullOrEmpty(value)){{return;}} obj.{0} = new Guid(value);\r\n"},
			{"System.DateTimeOffset","\r\nobj.SetPropertyDefaultValue(\"{0}\");if (string.IsNullOrEmpty(value)){{return;}} DateTimeOffset tmp;if(DateTimeOffset.TryParse(value, out tmp)){{obj.{0}=tmp;}};\r\n"},
        ...
        private string GetDataEntitySetCode(DbMapInfo info)
        {
            ...
            {
                return string.Format("obj.{0} = ({1})Convert.ToInt32(value);", info.NetName, typeName);
            }
        ...
        private string GetXmlEntitySetCode(DbMapInfo info)
        {
            ...
            if (typeName == "System.Byte[]"
                && info.AttrColumn != null
                && info.AttrColumn.TimeStamp)
            {
